"""
Thermodynamics of RBM learning -- second round of analyses (results-2).
...
def main():
    ...
    p.add_argument("--dir500", default=os.path.join(HERE, "runs_500ep"))
    p.add_argument("--dir5000", default=os.path.join(HERE, "runs_5000ep"))
    ...
    lines += ["Phase space (500 epochs): E in [%.3f, %.3f], S in [%.4f, %.4f] (ln 256 = %.4f), "
              "F in [%.3f, %.3f]" % (ranges.E["min"], ranges.E["max"], ranges.S["min"],
                                     ranges.S["max"], np.log(256), ranges.F["min"], ranges.F["max"]),
    ...
    lines += ["Long runs (10 seeds x 5000 epochs)",
              ...
              "KL: epoch 1 %.3f  ->  epoch 5000 %.3f +/- %.3f   (uniform model: ln(16/6) = %.3f)"
              % (d5000[d5000.epoch == 1].KL.mean(), d5000[d5000.epoch == 5000].KL.mean(),
                 d5000[d5000.epoch == 5000].KL.std(), np.log(16 / 6)),
    ...
    lines.append("Identity: KL = beta*(F_data - F) - ln 6, so F enters the learning objective "
                 "with a MINUS sign.")
