...
class RBM():
    def __init__(self, visible_dim, hidden_dim, number_of_epochs, picture_shape, batch_size, initial_temperature = 1,temps = [1],annealing_decay = 0, training_algorithm='cd', initializer='glorot', k=1, n_test_samples=500,small_Big=False, NAME='RBM',l_1 = -1E-6,non_parallel = False,lamda=1,KL_mode=False):
    ...
    def save_model_W(self):
        ...
        weights.numpy() \ biases.numpy() problems with numpy version no longer necessary have to check if it worked.
        """
        model_dict_save_W = {'weights': self.weights_W, 'visible_biases': self.visible_biases_W,
                           'hidden_biases': self.hidden_biases_W}
        d_m_folder= 'D:\Desktop\Spring 2024\ML\RBM_NEEDS_TO_WORK\\results\\models'+datetime.datetime.now().strftime("%d%m")
        if not os.path.exists(d_m_folder):
            os.makedirs(d_m_folder)
        h5direc = d_m_folder+'/'+self._current_time+'model.h5'
    ...
    def True_Z(self,v_all,h_all,Beta):
        ...
        for i in v_all:
            for j in h_all:
                Z+= np.exp((np.inner(i, self.visible_biases) +np.inner(j,self.hidden_biases) +np.inner(j, tf.tensordot(self.weights,i,1)))*Beta)
        return Z[0]
    ...
    def train(self, data, optimizer):
        ...
        for epoch in range(1,self._n_epoch+1):
            ...
            if self.small_Big:
                ...
                print(self._log_dir)
                print("epoch %d" % (epoch + 1),"Rec error: %s" % np.asarray(rec_error),"sq_error %s" % np.asarray(sq_error),"DKL: %.4f" % DKL,"Expected Energy %.5f" % EE,"Free Energy %.5f" % F_full, "W metric %.5f" % self.W_distance, "Entropy %.5f" % Entropy, "Beta %.5f" % Beta, "E-F+S %.5f" % Tot )
            else: 
                print(self._log_dir)
